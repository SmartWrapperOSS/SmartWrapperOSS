...
_JS = """
  ...
  function formatDimensionValue(dim) {
    if (!dim) return "—";
    // latency/cost dimensions carry a more readable string in `reason`
    // (e.g. "1.8s", "$0.018") — same convention as output/formatter.py.
    return dim.score;
  }
  ...
  function renderHead() {
    ...
    const cols = [
      ...
      ...dims.map(d => ({ key: d, label: d.replace(/_/g, " ") })),
  ...
  function renderTable() {
    ...
    RESULTS.forEach(r => {
      ...
      const dimCells = dims.map(d => `<td>${formatDimensionValue(r.dimensions[d])}</td>`).join("");
              ...
              ${dims.map(d => {
                ...
                return `
                  <div class="detail-card">
                    <div class="detail-card-label">${escapeHtml(d.replace(/_/g, " "))}</div>
